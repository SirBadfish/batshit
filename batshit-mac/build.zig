...
pub fn build(b: *std.Build) void {
    ...
    const package = b.addSystemCommand(&.{
        ...
        "--manifest",
        packageManifestPath(web_engine),
    ...
    if (platform == .macos) {
        switch (web_engine) {
            ...
            .chromium => {
                ...
                const sdk_include = if (b.sysroot) |sysroot| b.fmt("{s}/usr/include", .{sysroot}) else "";
                const flags: []const []const u8 = if (b.sysroot) |sysroot| &.{ "-fobjc-arc", "-ObjC++", "-std=c++17", "-stdlib=libc++", "-Wno-nullability-completeness", "-mmacosx-version-min=11.0", "-isysroot", sysroot, "-isystem", sdk_include, include_arg, define_arg } else &.{ "-fobjc-arc", "-ObjC++", "-std=c++17", "-stdlib=libc++", "-Wno-nullability-completeness", "-mmacosx-version-min=11.0", include_arg, define_arg };
...
fn webEnginePackageSuffix(web_engine: WebEngineOption) []const u8 {
    return switch (web_engine) {
        .system => "",
        .chromium => "-Chromium",
    };
}

fn packageManifestPath(web_engine: WebEngineOption) []const u8 {
    return switch (web_engine) {
        .system => "app.zon",
        .chromium => "app.chromium.zon",
    };
}
