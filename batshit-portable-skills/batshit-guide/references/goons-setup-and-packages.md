...
- A supported first-party file may intentionally correct geometry without changing topology. The file must identify the exact affected mesh, control component, and geometry channels; declare whether the correction changes the neutral/base surface or the relative control effects; and bind that claim to independent source/target audits while behavior and the component graph stay unchanged. Batshit still recomputes both the relative controls and complete baked result, keeps every saved value exact, and requires **Current / Updated** review; an extra, missing, or unrelated change remains blocked.
...
An exact first-party package can also keep its nipple/areola artwork circular on the body while chest and breast controls reshape the underlying surface. Batshit moves only the masked pigment onto a small nipple-centered skin area while leaving the body artwork—including painted breast shadows—on its original mapping. Only the skin surface connected to the physical nipple can receive the projected pigment, so another fold of the breast cannot display a duplicate piece. **Nipple Size** changes the physical nipple base ring and visible circle together at the smaller and neutral settings, then gives the artwork a wider positive range so the largest setting can produce a much broader areola without applying that extra scale to the central nipple. **Nipple Point** extends or flattens only the central nipple without moving, resizing, or reshaping the areola. The result remains painted on the body, so normal perspective can make the surface circle look narrower from an angle—it never turns into a camera-facing sticker. A package whose geometry, UVs, anchors, or material no longer match its signed projection is rejected clearly instead of falling back to squashed artwork.
