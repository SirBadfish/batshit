...
Mood, Emote, Motion, camera, Eye Contact behavior, voice, and ordinary runtime-only wardrobe/settings changes do not rebuild the Goon. If a later appearance update fails, the previously working Goon stays active. A failed first preparation remains clearly marked and offers Retry/Discard recovery instead of looking ready. If the app closes after a new appearance passes its checks but before activation, the Goon reopens at **Appearance Update Ready**. Choose **Finish Update** to apply that exact checked candidate, or **Discard** to remove only the unfinished candidate.
