...
For `API` agents, Batshit normalizes both formats into the same reasoning panel when the preset is marked reasoning-capable or the model/provider is a known MiMo or DeepSeek path. **Display Reasoning** controls whether that panel is shown, and **Preserve Reasoning** controls whether it is saved with the message and remains after refresh. Preserved reasoning is UI history only: Batshit does not insert it into later model requests, so it does not appear under **Compiled Messages** in the Execution Viewer. The Viewer's final-output section reports whether a reasoning summary was saved for the run. The tags and their contents do not become part of the normal assistant answer.
