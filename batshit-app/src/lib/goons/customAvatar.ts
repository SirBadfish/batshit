...
export type GoonCustomFaceManifest = {
  ...
  controls?: Partial<Record<BatshitFaceControlId | (string & {}), GoonCustomFaceControlBindingValue>>
...
const customManifestCache = new Map<string, Promise<GoonCustomAvatarManifest>>()
...
function normalizeNameList(value: GoonCustomMorphBindingValue | null | undefined) {
...
export function resolveCustomNamedNode(root: Object3D, name: string | null | undefined) {
...
export function resolveGoonKind(goon: Pick<GoonRecord, 'kind'> | null | undefined): GoonKind {
...
export function resolveGoonSourceProfile(
  ...
  return resolveGoonKind(goon) === 'custom' ? 'expert-custom-glb' : 'standard-vrm'
}
...
export function resolveGoonEyeContactMode(
  goon:
    | Pick<GoonRecord, 'kind' | 'sourceProfile' | 'defaults'>
    | null
    | undefined,
  ...
  const globalMode = normalizeGoonGlobalEyeContactSettingsMap(goonsSettings?.kitchen?.eyeContact)[
    profile
  ]?.mode
  if (globalMode === 'bone' || globalMode === 'expression') return globalMode
  return resolveGoonSourceProfile(goon) === 'guided-custom-vrm' ? 'expression' : 'bone'
}

export function normalizeGoonEyeContactMultiplier(value: unknown, fallback = 1): number {
  const numeric = typeof value === 'number' ? value : Number(value)
  if (!Number.isFinite(numeric)) return fallback
  return Math.max(0, Math.min(8, Math.round(numeric * 100) / 100))
}

export const normalizeGoonEyeContactStrength = normalizeGoonEyeContactMultiplier
...
export function normalizeGoonEyeContactHeadStart(value: unknown, fallback = 14): number {
  const numeric = typeof value === 'number' ? value : Number(value)
  if (!Number.isFinite(numeric)) return fallback
...
export function resolveGoonEyeContactGlobalProfile(
  goon:
    | Pick<GoonRecord, 'kind' | 'sourceProfile'>
    | null
    | undefined
): GoonEyeContactGlobalProfile {
  return resolveGoonSourceProfile(goon) === 'standard-vrm' ? 'vroid' : 'blender'
}
...
export function normalizeGoonGlobalEyeContactSettings(
  ...
  return {
    mode: value?.mode === 'bone' || value?.mode === 'expression' ? value.mode : fallbackMode,
...
export function normalizeGoonGlobalEyeContactSettingsMap(
  ...
  return {
    ...
    blender: normalizeGoonGlobalEyeContactSettings(value?.blender, 'expression')
  }
}
...
export function resolveGoonEyeContactTuning(
  ...
  const profile = resolveGoonEyeContactGlobalProfile(goon)
  const globalSettings = normalizeGoonGlobalEyeContactSettingsMap(goonsSettings?.kitchen?.eyeContact)
  return globalSettings[profile].tuning
}

export function resolveCustomManifestFile(goon: GoonRecord | null | undefined): GoonFileRef | null {
  if (resolveGoonKind(goon) !== 'custom') return null
  return goon?.customAvatar?.manifest ?? null
}

export function resolveGuidedManifestFile(goon: GoonRecord | null | undefined): GoonFileRef | null {
  if (!isGuidedCustomVrmGoon(goon)) return null
  return goon?.guidedAvatar?.manifest ?? null
}

export function resolveGoonManifestFile(goon: GoonRecord | null | undefined): GoonFileRef | null {
...
export function resolveGoonAvatarSignature(goon: GoonRecord | null | undefined) {
  if (!goon) return ''
  ...
  if (sourceProfile === 'guided-custom-vrm') {
    const overlayUrls = (goon.guidedAvatar?.dufOverlays ?? []).map((overlay) => overlay.file?.url ?? '')
    const pieceStates = goon.guidedAvatar?.pieceStates ?? {}
    const hasGuidedOutfitState = overlayUrls.length > 0 || Object.keys(pieceStates).length > 0
    return [
      kind,
      sourceProfile,
      avatarUrl,
      goon.guidedAvatar?.manifest?.url ?? '',
      ...(hasGuidedOutfitState ? [...overlayUrls, JSON.stringify(pieceStates)] : [])
    ].join('::')
  }
...
export function resolveCustomExpressionBindings(manifest: GoonCustomAvatarManifest) {
...
export function resolveCustomMorphDefinitions(manifest: GoonCustomAvatarManifest) {
  const definitions: ResolvedCustomMorphDefinition[] = []

  for (const [id, rawValue] of Object.entries(manifest.face?.customMorphs ?? {})) {
...
function looksLikeCustomAvatarManifest(value: unknown): value is GoonCustomAvatarManifest {
  return (
    isRecord(value) &&
    ('stage' in value ||
      'body' in value ||
...
function normalizeEmbeddedManifestPayload(value: unknown): GoonCustomAvatarManifest | null {
...
export function extractEmbeddedCustomAvatarManifest(document: unknown): GoonCustomAvatarManifest | null {
...
export function resolveCustomFaceMeshes(
  ...
  if (requestedMeshNames.length > 0) {
    for (const meshName of requestedMeshNames) {
      ...
      if (!node) {
        issues.push(`Custom face mesh "${meshName}" was not found in the model.`)
        continue
      }

      const resolved = collectMorphEnabledDescendants(node)
      if (resolved.length === 0) {
        issues.push(`Custom face mesh "${meshName}" did not resolve to any morph-enabled meshes.`)
        continue
      }
...
export async function loadCustomAvatarManifest(
  ...
  const request = (async () => {
    ...
    if (!response.ok) {
      throw new Error(`Failed to load Custom avatar manifest (${response.status}).`)
    }
...
export async function loadAvatarIntoEngine(engine: GoonEngine, goon: GoonRecord) {
  const kind = resolveGoonKind(goon)
  const sourceProfile = resolveGoonSourceProfile(goon)
  const avatarUrl = resolveGoonAvatarUrl(goon)
  if (!avatarUrl) {
    throw new Error(
      kind === 'custom' ? 'Custom Goon is missing its model file.' : 'Goon is missing its VRM file.'
    )
  }
  ...
  if (sourceProfile === 'guided-custom-vrm') {
    const manifest = await loadCustomAvatarManifest(resolveGuidedManifestFile(goon))
