...
/**
 * SA-113 P2 (DL-113-04a) — the code fallback for `batshit_dm_guidance.md`.
 *
 * Same contract as every other block here: the packaged Markdown is what ships and what
 * Admin edits, and this is what compiles when the Redis key is empty. A wording change
 * must touch BOTH, or an instance whose prompt was never seeded gets different
 * instructions from one that was — the exact drift `toolPromptInjection.test.ts` pins.
 *
 * No runtime split: the `sys.dm.*` control ids are the same on the API lane and the
 * managed CLI lanes, because both reach them through the Batshit Tool Search/Use broker.
 */
export function buildDmGuidancePromptBlock(): string {
  return [
    ...
    '- `wake` asks Batshit to start a turn for them now. Use it only for work that should start now, and expect refusals — wake-ups off, already mid-task, over an hourly limit. A refused wake becomes a `wait` with a reason; nothing is lost and nothing retries.',
    '- `steer` lands your DM inside the reply they are writing right now, at their next tool call. Use it when a correction now saves the whole reply, and only when `sys.dm.agents` shows them `running`. It starts no turn and spends no wake budget, and one reply holds one agent DM.',
    '- `steer_fallback` is `wait` (the default) or `wake` for when they are not mid-reply; `delivered_as` says which you got. A steer that never reaches a tool boundary becomes a `wait`, never a message from the user.',
    '- Ask `sys.dm.agents` who is free before choosing.',
    '',
    'Your inbox:',
    ...
    '- `sys.dm.read` opens one and `sys.dm.claim` takes an assignment — both take `dm_id`, and a claim is **one at a time**, so finish the one you are on first.',
    '- `sys.dm.done` or `sys.dm.blocked` closes it and both take `dm_id` and `result`. The result text you write IS the answer sent back, so make it real.',
    '',
    'Your own schedules:',
