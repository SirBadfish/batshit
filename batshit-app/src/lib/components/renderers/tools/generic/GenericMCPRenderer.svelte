<!-- GenericMCPRenderer.svelte - Professional renderer for MCP Gateway tools -->
<script lang="ts">
  ...
  import { formatToolDisplayName } from '$lib/utils/toolNameFormatter'
	...
	// ===== SOURCE BADGE HELPER =====
	// Same as GenericToolRenderer for consistency
  ...
  let toolDisplayName = $derived(tool.displayToolName || formatToolDisplayName(tool.toolName))
	...
	let formattedResult = $derived(formatMCPResult(tool.toolResult))
	...
	function extractMeaningfulData(obj: any): any {
		...
		const meaningfulFields = ['result', 'output', 'data', 'content', 'response']
		for (const field of meaningfulFields) {
			if (obj[field] !== undefined) return obj[field]
		}

		// Filter out technical noise
		const technicalFields = ['id', 'type', 'timestamp', 'metadata', 'status']
	...
	function formatMCPResult(result: any, depth = 0): any {
		...
		// Handle strings (PERF-001: Truncate at 10K chars)
		...
		// Handle arrays (PERF-001: Truncate at 100 items)
