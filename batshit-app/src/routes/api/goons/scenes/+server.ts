...
export const POST: RequestHandler = async ({ request, locals }) => {
  ...
  const uploadResponse = await fetch(`${getInternalBatshitServerUrl()}/api/upload/goon-scene`, {
    method: 'POST',
  ...
  if (!uploadResponse.ok) {
    ...
    return json({ error: text || 'Scene upload failed' }, { status: uploadResponse.status })
  }
...
export const DELETE: RequestHandler = async ({ request, locals }) => {
  ...
  const response = await fetch(`${getInternalBatshitServerUrl()}/api/upload/goon-scene`, {
    method: 'DELETE',
  ...
  if (!response.ok) {
    ...
    return json({ error: text || 'Scene delete failed' }, { status: response.status })
  }
