...
export const POST: RequestHandler = async ({ request, locals }) => {
  ...
  const uploadResponse = await fetch(`${getInternalBatshitServerUrl()}/api/upload/goon-scene-prop`, {
    method: 'POST',
  ...
  if (!uploadResponse.ok) {
    ...
    return json({ error: text || 'Scene prop upload failed' }, { status: uploadResponse.status })
  }
...
export const DELETE: RequestHandler = async ({ request, locals }) => {
  ...
  const response = await fetch(`${getInternalBatshitServerUrl()}/api/upload/goon-scene-prop`, {
    method: 'DELETE',
  ...
  if (!response.ok) {
    ...
    return json({ error: text || 'Scene prop delete failed' }, { status: response.status })
  }
