...
export const POST: RequestHandler = async ({ request, locals }) => {
  ...
  const uploadResponse = await fetch(`${getInternalBatshitServerUrl()}/api/upload/goon-room-texture`, {
    method: 'POST',
  ...
  if (!uploadResponse.ok) {
    ...
    return json({ error: text || 'Room texture upload failed' }, { status: uploadResponse.status })
  }
...
export const DELETE: RequestHandler = async ({ request, locals }) => {
  ...
  const response = await fetch(`${getInternalBatshitServerUrl()}/api/upload/goon-room-texture`, {
    method: 'DELETE',
  ...
  if (!response.ok) {
    ...
    return json({ error: text || 'Room texture delete failed' }, { status: response.status })
  }
