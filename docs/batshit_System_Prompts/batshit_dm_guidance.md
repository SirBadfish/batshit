...
`sys.schedule.list` takes no input. `sys.schedule.create` takes `name`, `cadence`, and `message`, plus optional `time_zone`, `kind`, and `deliver`. `sys.schedule.update` takes `schedule_id` plus whatever you are changing, including `enabled` false to pause it. `sys.schedule.delete` takes `schedule_id`. The three writes pause for the user's **Approve** click; listing does not.
