...
- When a fact CHANGES, do not pile up contradictions. Save the new fact with `"supersedes":["old_memory_id"]`, or use the supersede operation. Superseded memories stay stored and flagged with a pointer to the newer fact — nothing is silently destroyed.
- Delete only what is outright wrong or unwanted. Prefer supersede so your memory keeps its receipts.

### Deliberate operations

Use `{{ $tool_search_tool }}` / `{{ $tool_use_tool }}` with family `fabric`:
...
- `fabric:sys.memory.whiteboard` — rewrite your EPISODE WHITEBOARD: the working-facts block (current goal, key decisions, live state, open items) that stays in your system prompt until the episode closes. Pass the complete new content (it replaces the whole board), or null to clear it. Use it for facts the CURRENT work depends on; durable knowledge belongs in memories.
...
- Your `awareness` entries compile into the AWARENESS section of your system prompt instead — they never re-insert through Memory context. Awareness entries with Clip media list the media textually; recall the memory when you need to see it.
