...
- Current first-party socket-eye Goons keep **Eye Corner Smoothing**, linked **Iris Size**, iris-relative **Pupil Size**, and linked **Iris Vertical Position** under **Face Appearance -> Eyes**. Iris and Pupil Size use `0.5..1.5` with `1` as neutral; hiding the pupil is a visibility choice, not a zero-size endpoint. Iris Vertical Position moves Iris and Pupil together without moving fixed Highlight or the separately gaze-linked Sclera artwork. They expose no Sclera Fit. **Eye Contact** contains exactly Enabled, Strength, Head Follow, Response, and Gaze Convergence; one world target is projected independently into both eye surfaces for automatic depth-aware convergence, while Gaze Convergence provides only a small bounded fine-tune. The cap stays fixed under Head and retains exact Blink/Wide/Squint rather than rotating a globe; the liner retains its exact authored `44` facial-performance targets. Standard/VRoid and Advanced/Blender eye behavior is unchanged.
- Facial Artwork v6 covers bilateral Brows, Lashes & Eye Outline, independent Iris and Pupil, Eye Highlight, and Sclera layers. The editor requires single-frame 8-bit PNGs at the exact package-template dimensions and never resizes them. Users download one side-aware Template, paint on a separate transparent layer, and hide/remove the Template before export. Upload preparation converts pixels to canonical sRGB/RGBA, removes embedded ICC metadata, clips alpha to the trusted internal Mask, clears invisible fringe color, and losslessly re-encodes before strict template/provenance validation. The first-party physical eye is one static full sphere per side: eyelids and treatment animate over it, and the sphere has no Blink/Wide/Squint morphs. Highlight is a fixed, unmirrored front/cornea-space graphic with no Iris-edge crop; Sclera uses a full-sphere equirectangular map that rotates with gaze and keeps its artist-authored longitude rotation as a separate offset. Do not advise users to strip an ordinary ICC profile manually, paint arbitrary dimensions, deform the physical sphere for expressions, or bypass package validation.
...
- **Update Goon File** checks a newer supported `.bgoon`. The actions are **Update Goon**, **Keep Current**, or an explicitly confirmed **Reset Appearance and Update**. A verified intentional presentation change requires matched Current / Updated review before Update Goon. Blocked/ineligible files never replace the active Goon.
...
- At the exact Eye v4 / Facial Artwork v5 migration edge, only the **Current** comparison preview may render those legacy saved siblings. **Updated** and committed runtime are Eye v5 / Facial Artwork v6 only. The Eye conversion applies `irisSize / 1.35`, `pupilSize / 1.40`, horizontal `+0.50`, and vertical `+0.70`; Facial Artwork preserves uploads, colors, visibility, shared/per-eye choice, provenance, tint, opacity, Brow/Lashes/Highlight transforms, Iris/Pupil rotation, and Sclera rotation while rebinding exact templates and canonicalizing removed Iris/Pupil artwork translation/scale to identity. Any wrong schema, definition, template, hash, bounds, extra field, or ambiguity blocks instead of falling back.
...
- Facial Artwork Template downloads, validated PNG replacement, Same for both/Customize each eye controls, solid eye colors, role-specific artwork transforms/tint/opacity, calibrated physical Iris/Pupil size, and reset for supported first-party Advanced/GLB packages
- immutable built-in and user-imported Hair selection, color, and motion settings; built-ins stay non-editable/non-deletable, while user imports keep their own Edit/refit flow
...
- all six roles can be shared or use explicit per-eye values; shared Brows, Lashes & Eye Outline, Iris, Pupil, and Sclera use the accepted mirror behavior, while shared Highlight is unmirrored so its source orientation stays the same in both eyes; linked Brow Horizontal Position moves both brows inward/outward rather than sliding them in one screen direction, and the widened brow transforms intentionally permit extreme placement; the Brow template/physical regions meet exactly at the face center for true unibrow art and extend farther outward and upward
- Iris, Pupil, and Sclera keep solid base colors when no PNG is present; optional artwork composes over the base
- Brows, Lashes & Eye Outline, and Highlight expose artwork position, scale, and rotation; Iris/Pupil expose artwork rotation only; Sclera exposes only its longitude artwork rotation
- Iris and Pupil remain independent artwork roles; both size controls use `0.5..1.5`, neutral `1`, Pupil Size remains iris-relative, and visibility is the supported hide path
- Eye Highlight is fixed in front/cornea space, does not follow gaze or Iris placement, may cross onto Sclera, and uses eyelid/treatment depth rather than Iris-mask cropping
- Sclera artwork uses a full-sphere 2:1 map, rotates with gaze, wraps longitude seam-safely, and keeps Artwork Rotation as an additive artist offset
- Save Goon atomically owns the version-bound Facial Artwork v6 and Eye Appearance v5 states. The sole exact older-state migration exists only inside the declared Recipe update and its Current comparison preview; do not treat it as ordinary runtime compatibility.

Built-in Hair truth:
- the versioned built-in catalog retains immutable historical revisions, declares one current revision per style, and permits only explicit same-style successor edges
- an exact saved built-in selection may advance through its declared successor path while preserving Base/Highlight colors and motion settings; missing, tampered, ambiguous, or motion-incompatible paths fail loudly
- `None`, already-current built-ins, and every user-imported selection remain unchanged; user imports are never auto-migrated as built-ins
- built-ins remain non-editable and non-deletable; stale user imports keep their separate explicit Edit/refit path that publishes a new immutable user revision
